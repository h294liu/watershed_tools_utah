#!/usr/bin/env python
# coding: utf-8
# Genevieve Brown -- radiation code
# watershed_tools implementation -- Hongli Liu, 2020; Andy Wood, 2021
...
def process_command_line():
    ...
    parser = argparse.ArgumentParser(description='Script to calculate 24-hr radiation.')
    ...
    parser.add_argument('epsg', help='EPSG integer to assign coordinate systems and transformations.')
...
def calculate_Sw(latitude, elev, slope, aspect, DOY):
    # Calculate solar radiation given elevation, slope and aspect
...
if __name__ == '__main__':
    ...
    Sw_DOY = calculate_Sw(lat_ma,elev_ma,slp_ma,asp_ma,DOY)
    # Sw_DOY is a masked array because all inputs are masked arrays.
    
    # save Sw as raster
    print('save Sw raster')
    Sw_DOY_value = Sw_DOY.filled(fill_value=nodatavals)  # return copy with masked values filled with a given value
    ...
    with rio.open(os.path.join(output_path,ofile_raster), 'w', **out_meta) as outf:
